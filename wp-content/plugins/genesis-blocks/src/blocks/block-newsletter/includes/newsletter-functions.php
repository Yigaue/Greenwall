...
add_action( 'wp_ajax_genesis_blocks_newsletter_submission', __NAMESPACE__ . '\form_submission_listener' );
add_action( 'wp_ajax_nopriv_genesis_blocks_newsletter_submission', __NAMESPACE__ . '\form_submission_listener' );
/**
...
function form_submission_listener() {
	...
	if ( empty( $_POST['gb-newsletter-form-nonce'] ) ) {
		return;
	}

	if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['gb-newsletter-form-nonce'] ) ), 'gb-newsletter-form-nonce' ) ) {
		send_processing_response( __( 'Nonce verification failed. Please try again.', 'genesis-blocks' ) );
	}

	if ( empty( $_POST['gb-newsletter-mailing-list-provider'] ) || empty( $_POST['gb-newsletter-mailing-list'] ) ) {
		send_processing_response( __( 'Invalid mailing provider configuration.', 'genesis-blocks' ) );
	}

	if ( empty( $_POST['gb-newsletter-email-address'] ) ) {
		send_processing_response( __( 'You must provide an email address.', 'genesis-blocks' ) );
	}

	$email              = sanitize_email( wp_unslash( $_POST['gb-newsletter-email-address'] ) );
	$provider           = sanitize_text_field( wp_unslash( $_POST['gb-newsletter-mailing-list-provider'] ) );
	$list               = sanitize_text_field( wp_unslash( $_POST['gb-newsletter-mailing-list'] ) );
	$default_attributes = genesis_blocks_newsletter_block_attributes();
	$success_message    = ! empty( $_POST['gb-newsletter-success-message'] ) ? sanitize_text_field( wp_unslash( $_POST['gb-newsletter-success-message'] ) ) : $default_attributes['successMessage']['default'];
	$subscriber_status  = ! empty( $_POST['gb-newsletter-double-opt-in'] ) ? 'pending' : 'subscribed';

	if ( ! is_email( $email ) ) {
		send_processing_response( __( 'Please provide a valid email address.', 'genesis-blocks' ) );
	}
...
function send_processing_response( $message, $success = true ) {
	...
	if ( function_exists( 'is_amp_endpoint' ) && ! empty( $_POST['gb-newsletter-amp-endpoint-request'] ) ) {
		...
		if ( $location ) {
			$redirect_url = add_query_arg(
				[
					'gb-newsletter-submitted'          => true,
					'gb-newsletter-submission-message' => rawurlencode( $message ),
				],
				$location
			);
...
function process_submission( $email, $provider, array $args ) {
	...
	$errors = [
		'invalid_provider' => esc_html__( 'Invalid newsletter provider.', 'genesis-blocks' ),
		'invalid_email'    => esc_html__( 'Invalid email address.', 'genesis-blocks' ),
		'invalid_list_id'  => esc_html__( 'Invalid list ID.', 'genesis-blocks' ),
		'invalid_api_key'  => esc_html__( 'You must enter your email API key in the plugin settings page.', 'genesis-blocks' ),
	];
	...
	switch ( $provider ) {
		...
		case 'mailchimp':
			$api_key = get_option( 'genesis_blocks_mailchimp_api_key' );
...
function mailing_list_providers() {
	...
	$mailchimp_api_key = get_option( 'genesis_blocks_mailchimp_api_key' );
	...
	return [
		'mailchimp' => [
			...
			'api_key_defined' => ! empty( get_option( 'genesis_blocks_mailchimp_api_key' ) ),
		],
	];
}
...
function frontend_assets() {
	wp_register_script(
		'genesis-blocks-newsletter-functions',
		plugins_url( '/dist/assets/js/newsletter-block-functions.js', genesis_blocks_main_plugin_file() ),
		[ 'jquery', 'wp-a11y' ],
		'1.0',
		true
	);
	...
	wp_localize_script(
		'genesis-blocks-newsletter-functions',
		'genesis_blocks_newsletter_vars',
		[
			'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
			'l10n'    => [
				'button_text_processing' => esc_html( genesis_blocks_newsletter_block_attributes()['buttonTextProcessing']['default'] ),
				'invalid_configuration'  => esc_html__( 'Invalid configuration. Site owner: Please configure your newsletter provider settings.', 'genesis-blocks' ),
				'a11y'                   => [
					'submission_processing' => esc_html__( 'The submission is being processed.', 'genesis-blocks' ),
					'submission_succeeded'  => esc_html__( 'The submission successfully succeeded.', 'genesis-blocks' ),
					'submission_failed'     => esc_html__( 'The submission failed.', 'genesis-blocks' ),
				],
			],
		]
	);
